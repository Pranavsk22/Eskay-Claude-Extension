(() => {
  ...
  history.pushState = function (...args) {
    ...
    window.dispatchEvent(new CustomEvent('ek:urlchange'));
    return result;
  };
  ...
  function post(type, payload) {
    window.postMessage({ ek: EK_MARKER, type, payload }, '*');
  }
  ...
  async function handleConversationResponse({ orgId, conversationId }, response) {
    try {
      ...
      post('ek:conversation', { orgId, conversationId, data });
    } catch {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (json?.type === 'message_limit' && json.message_limit) {
              post('ek:message_limit', json.message_limit);
            }
          } catch {
            // ignore
          }
  ...
  window.addEventListener('message', async (event) => {
    ...
    if (!data || data.ek !== EK_MARKER) return;
    if (data.type !== 'ek:request') return;
    ...
    try {
      ...
      if (kind === 'conversation') {
        ...
        const json = await res.json();
        post('ek:conversation', { orgId, conversationId, data: json });
        postResponse(requestId, true, json, null);
        return;
      }
